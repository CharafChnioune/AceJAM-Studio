...
- On Intel Mac with Python 3.10, the launcher uses the older `torch==2.2.2` stack, pins `numpy==1.26.4`, `diffusers==0.31.0`, `numba==0.61.2`, and `vector-quantize-pytorch==1.25.0`, and builds `llama-cpp-python` from source because the newer wheel lines used elsewhere are no longer published for `macos x86_64`.
