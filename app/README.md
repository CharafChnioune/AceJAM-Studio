...
The status payload now reports `cli_features` as well: MLX Media probes whether the installed MFLUX build supports the newer atomic `--lora PATH SCALE` and `--image PATH [STRENGTH]` flags that landed on upstream `main` on June 24, 2026. Command building uses those forms when present and otherwise keeps the `0.18.x` legacy flag layout so current released installs remain compatible.
...
- `negative_prompt` and `no_negative_prompt` now surface the Wan classifier-free guidance controls directly in the Video wizard.
- `trim_first_frames`, `end_image_strength`, and `audio_start_time` are now first-class payload fields for Wan Lightning and LTX image/audio conditioning workflows.
- Video-LoRA roles are editable in the wizard so Wan2.2 high-noise / low-noise pairs can be wired exactly like the upstream examples.
...
```json
{
  ...
  "prompt": "moody live performance, fast push-ins, punchy edits",
...
All LoRA/LoKr train commands save every epoch. The Trainer runs PEFT LoRA epoch auditions by default: one no-LoRA baseline is generated first, then a fixed 30-second, full-quality 64-step LoRA WAV generation runs after each epoch checkpoint. The job keeps the baseline plus the latest three LoRA auditions in `epoch_auditions`, so the current training quality is always listenable without filling the job with every old epoch. The audition uses clean genre-default lyrics selected from the Test genre dropdown next to the trigger tag; the UI shows the exact standard lyrics in a read-only preview, removes the old free-form test-lyrics entry, and adds the user's trigger tag to the caption only. These audition renders are job artifacts only and are not saved to the Music Library; LoKr auditions are skipped because standard ACE-Step generation expects PEFT LoRA adapter files.
