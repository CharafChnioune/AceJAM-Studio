...
def _mps_dtype_supported(dtype_name: str) -> bool:
    ...
    if dtype is None:
        return False
    try:
        probe = torch.ones((2, 2), device="mps", dtype=dtype)
        result = probe + probe
        _ = result.cpu()
        return True
    except Exception as exc:
        logger.warning(f"[initialize_service] MPS dtype probe failed for {dtype_name}: {exc}")
        return False
...
class AceStepHandler:
    ...
    def _requested_dtype(self) -> tuple[str, Optional[torch.dtype]]:
        value = os.environ.get("ACE_STEP_DTYPE", "auto").strip().lower()
        if not value:
            value = "auto"
        dtype = DTYPE_ALIASES.get(value)
        if value not in DTYPE_ALIASES:
            logger.warning(f"[initialize_service] Unknown ACE_STEP_DTYPE={value!r}; using auto")
            return "auto", None
        return value, dtype

    def _dtype_candidates(self, device: str) -> List[torch.dtype]:
        requested_name, requested_dtype = self._requested_dtype()
        if requested_dtype is not None:
            if device == "mps" and not _mps_dtype_supported(_dtype_name(requested_dtype)):
                raise RuntimeError(f"MPS does not support requested dtype: {requested_name}")
        ...
        if device == "mps":
            ...
            return candidates or [torch.float32]
        return [torch.float32]
    ...
    def initialize_service(
        ...
        try:
            ...
            requested_dtype_name, _ = self._requested_dtype()
            dtype_candidates = self._dtype_candidates(device)
            self.dtype = dtype_candidates[0]
            ...
            if os.path.exists(acestep_v15_checkpoint_path):
                ...
                last_error = None
                selected_dtype = self.dtype
                for candidate_dtype in dtype_candidates:
                    self.dtype = candidate_dtype
                    selected_dtype = candidate_dtype
                    candidate_name = _dtype_name(candidate_dtype)
                    for attn_impl in attn_fallback_order:
                        try:
                            logger.info(
                                f"[initialize_service] Attempting to load model with attention implementation: "
                                f"{attn_impl}, dtype: {candidate_name}"
                            )
                            ...
                            self.model = None
                            logger.warning(
                                f"[initialize_service] Failed to load model with {attn_impl}, dtype {candidate_name}: {e}"
                            )
            ...
            status_msg += f"Requested dtype: {requested_dtype_name}\n"
